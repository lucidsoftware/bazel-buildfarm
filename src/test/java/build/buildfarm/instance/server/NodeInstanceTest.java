...
public class NodeInstanceTest {
  ...
  @Test
  public void sharedSubtreeSymlinkScanIsMemoizedAcrossReferences() {
    // A subtree referenced by many siblings must have its symlink scan memoized to a single walk.
    // Enumeration re-fetches the shared digest once per reference (the floor); the scan must not
    // add
    // another fetch per reference, so the total stays ~referenceCount, not ~2*referenceCount.
    int referenceCount = 64;

    // Inner subtree: a single symlink-free directory with one file (no subdirectories of its own).
    Directory innerDirectory =
        Directory.newBuilder().addFiles(FileNode.newBuilder().setName("f").build()).build();
    build.bazel.remote.execution.v2.Digest innerDigest =
        DigestUtil.toDigest(DIGEST_UTIL.compute(innerDirectory));

    // Shared subtree references the inner directory once.
    Directory sharedDirectory =
        Directory.newBuilder()
            .addDirectories(DirectoryNode.newBuilder().setName("x").setDigest(innerDigest).build())
            .build();
    build.bazel.remote.execution.v2.Digest sharedDigest =
        DigestUtil.toDigest(DIGEST_UTIL.compute(sharedDirectory));

    // Root references the shared subtree referenceCount times under distinct, sorted names.
    Directory.Builder rootBuilder = Directory.newBuilder();
    for (int i = 0; i < referenceCount; i++) {
      rootBuilder.addDirectories(
          DirectoryNode.newBuilder()
              .setName(String.format("d%02d", i))
              .setDigest(sharedDigest)
              .build());
    }
    Directory rootDirectory = rootBuilder.build();
    ...
    Map<build.bazel.remote.execution.v2.Digest, Directory> directoriesIndex =
        new HashMap<>() {
          ...
          public Directory get(Object key) {
            if (innerDigest.equals(key)) {
              innerFetches[0]++;
            }
            return super.get(key);
          }
        };
    directoriesIndex.put(sharedDigest, sharedDirectory);
    directoriesIndex.put(innerDigest, innerDirectory);
    ...
    assertThat(preconditionFailure.getViolationsCount()).isEqualTo(0);
    // ~referenceCount (enumeration) + 1 memoized scan; a non-memoized scan would ~double it.
    assertThat(innerFetches[0]).isAtMost(referenceCount + 3);
  }
}
