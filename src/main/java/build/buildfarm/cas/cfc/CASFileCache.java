...
public abstract class CASFileCache implements ContentAddressableStorage {
  ...
  private static final Counter casLookups =
      Counter.build()
          ...
          .register();
  // Resolve the label children once: findMissingBlobs increments these per digest on a hot path.
  private static final Counter.Child casLookupHit = casLookups.labels("hit");
  private static final Counter.Child casLookupMiss = casLookups.labels("miss");
  ...
  private boolean contains(
    ...
    if (entry.isPlaceholder()) {
      try {
        size = Files.size(getPath(digest, key));
      } catch (IOException e) {
        return false;
      }
    }
    ...
    do {
      ...
      if (e != null) {
        ...
        try {
          ...
          InputStream fis = Files.newInputStream(getPath(digest, key));
          try {
    ...
    Write write =
        new Write() {
          ...
          private Path getWritePath() {
            ...
            return getPath(key.getDigest(), blobKey)
                .resolveSibling(blobKey + "." + key.getIdentifier());
          }
  ...
  private void loadSnapshotFile(
    ...
    try (BufferedReader br = Files.newBufferedReader(file)) {
      for (SizeEntry entry : db.entries(br)) {
        ...
        if (!tryReserveStartupBytes(key, diskSize)) {
          continue;
        }
        if (!files.remove(path)) {
  ...
  protected boolean tryReserveStartupBytes(String key, long diskSize) {
    checkArgument(diskSize >= 0, "diskSize (%s) must be non-negative", diskSize);
    // Reserve against the owning shard's per-shard cap to match runtime accounting, not a global
    // cap.
    return casShards.tryReserveStartupBytes(key, diskSize);
  }

  protected void releaseStartupBytes(String key, long diskSize) {
    casShards.releaseStartupBytes(key, diskSize);
  }
  ...
  private CacheScanResults scanRoot(Consumer<Digest> onStartPut)
    ...
    casShards.resetStartupAdmission();
    ...
    if (basename.endsWith("_dir")) {
      ...
      if (size > maxEntrySizeInBytes || size <= 0) {
        ...
        if (fileEntryKey == null) {
          ...
          if (!tryReserveStartupBytes(fileEntryKey.key(), diskSize)) {
            synchronized (deleteFiles) {
              deleteFiles.add(path);
            }
          } else {
            ...
            try {
              ...
              if (!admitted) {
                releaseStartupBytes(fileEntryKey.key(), diskSize);
              }
            }
          }
        }
  ...
  private CancellableOutputStream putOrReferenceGuarded(
    ...
    return new CancellableOutputStream(countingOut) {
      ...
      void commit() throws IOException {
        ...
        Path canonicalPath = CASFileCache.this.getPath(actual, key);
  ...
  private void performCopy(Write write, Digest digest, String key) {
    ...
    ListenableFuture<Long> future =
        WritesHelper.streamIntoWriteFuture(
            () -> Files.newInputStream(getPath(digest, key)), write, digest, 1, MINUTES);
    if (future.isDone()) {
      try {
        future.get();
      } catch (ExecutionException e) {
