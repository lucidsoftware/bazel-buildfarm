...
final class EvictorShard extends EvictorShardPad1 {
  ...
  /** Number of chargers currently parked at the hard cap. Lets tests await the parked state. */
  @VisibleForTesting
  long parkedAtHardCapForTesting() {
    return parkedAtHardCap.sum();
  }

  /**
   * Invoke the uncaught-exception handler as if the evictor thread had died. Lets tests exercise
   * the "shard dies while a charger is parked" path deterministically, without racing a real sweep.
   */
  @VisibleForTesting
  void simulateEvictorDeathForTesting(Throwable error) {
    onEvictorThreadDied(evictorThread, error);
  }
  ...
  private void parkUntilUnderHardCap(long bytesReserved) throws InterruptedException, IOException {
    ...
    try {
      while (shardBytes.sum() > parkBytes && !stopping && !dead) {
      ...
      if (dead) {
        // Evictor died while we were parked; nothing will drain the shard now. Roll back, re-wake
        // siblings, and fail fast with SHARD_DEAD rather than waiting out the hard-cap timeout.
        shardBytes.add(-bytesReserved);
        cond.signalAll();
        throw deadShardBackpressure();
      }
  ...
  private void onEvictorThreadDied(Thread thread, Throwable error) {
    dead = true;
    // Wake parked chargers; nothing else will signal them now, so they'd otherwise hang until the
    // hard-cap timeout. The park loop re-checks `dead` and fails them fast with SHARD_DEAD.
    signalParkers();
