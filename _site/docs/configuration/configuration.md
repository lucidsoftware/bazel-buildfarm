...
| linkedInputDirectories            | List of Strings, _empty_      |                       | **Deprecated and ignored.** Use `linkedInputExclusionPatterns`. |
| linkedInputExclusionPatterns      | List of Strings, _empty_      |                       | Full-match regexes for input directory paths that must remain real in addition to output paths. To migrate from the old inclusion setting, use complementary patterns; for example, `^(?!external$).*$` becomes `external`. Invalid regexes prevent worker startup. |
...
| evictorShards                | Integer (power of two), _0_   | Number of independent evictor shards the CAS is partitioned into; a key routes to a shard by hash(key) & (evictorShards - 1). 0 auto-derives from vCPU and worker role (CAS-only: nextPow2(vCPU/4); exec/combined: nextPow2(vCPU/8)) and may be clamped so each shard can hold maxEntrySizeBytes within the cache cap. Set 1 to preserve pre-sharding single-evictor behavior. Persisted in snapshot filenames; a changed value triggers a one-time snapshot migration on the next startup. |
